...
public class DataCacheUtils
{
     ...
     * 文件名，用户偏好信息保存的文件名
     */
    public static final String FILE_PREFERENCE = "user_preference_info";
    ...
    static class DataCache<T>
    {
        ...
        private void save(Context ctx, ArrayList<T> data, String name, String folder)
        {
            ...
            if (!folder.isEmpty())
            {
                File fileDir = new File(ctx.getCacheDir() + File.separator + ctx.getPackageName(), folder);
            ...
            {
                file = new File(ctx.getCacheDir() + File.separator + ctx.getPackageName(), name);
            }
            if (file.exists())
            {
                file.delete();
